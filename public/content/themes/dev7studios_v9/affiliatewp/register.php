...
<form id="affwp-register-form" class="affwp-form col-sm-6" action="" method="post">
	...
	<fieldset>
		...
		<?php if ( ! is_user_logged_in() ) : ?>
			...
			<div class="form-group">
				...
				<input id="affwp-user-name" class="required form-control" type="text" name="affwp_user_name" value="<?php if( ! empty( $user_name ) ) { echo $user_name; } ?>" title="<?php esc_attr_e( 'Your Name', 'affiliate-wp' ); ?>" />
			</div>

			<div class="form-group">
				<label for="affwp-user-login"><?php _e( 'Username', 'affiliate-wp' ); ?></label>
				<input id="affwp-user-login" class="required form-control" type="text" name="affwp_user_login" value="<?php if( ! empty( $user_login ) ) { echo $user_login; } ?>" title="<?php esc_attr_e( 'Username', 'affiliate-wp' ); ?>" />
			</div>

			<div class="form-group">
				<label for="affwp-user-email"><?php _e( 'Account Email', 'affiliate-wp' ); ?></label>
				<input id="affwp-user-email" class="required form-control" type="email" name="affwp_user_email" value="<?php if( ! empty( $user_email ) ) { echo $user_email; } ?>" title="<?php esc_attr_e( 'Email Address', 'affiliate-wp' ); ?>" />
			</div>
		...
		<?php endif; ?>
			...
			<div class="form-group">
				...
				<input id="affwp-payment-email" class="form-control" type="email" name="affwp_payment_email" value="<?php if( ! empty( $payment_email ) ) { echo $payment_email; } ?>" title="<?php esc_attr_e( 'Payment Email Address', 'affiliate-wp' ); ?>" />
			</div>

			<div class="form-group">
				<label for="affwp-user-url"><?php _e( 'Website URL', 'affiliate-wp' ); ?></label>
				<input id="affwp-user-url" class="form-control" type="text" name="affwp_user_url" value="<?php if( ! empty( $url ) ) { echo $url; } ?>" title="<?php esc_attr_e( 'Website URL', 'affiliate-wp' ); ?>" />
			</div>

			<div class="form-group">
				<label for="affwp-promotion-method"><?php _e( 'How will you promote us?', 'affiliate-wp' ); ?></label>
				<textarea id="affwp-promotion-method" class="form-control" name="affwp_promotion_method" rows="5" cols="30"><?php if( ! empty( $method ) ) { echo esc_textarea( $method ); } ?></textarea>
			</div>
		...
		<?php if ( ! is_user_logged_in() ) : ?>
			...
			<div class="form-group">
				...
				<input id="affwp-user-pass2" class="password required form-control" type="password" name="affwp_user_pass2" />
			</div>
		...
		<div class="form-group">
			...
			<input class="btn btn-primary" type="submit" value="<?php esc_attr_e( 'Register', 'affiliate-wp' ); ?>" />
		</div>
